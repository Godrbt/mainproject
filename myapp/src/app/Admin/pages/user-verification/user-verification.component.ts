...
export class UserVerificationComponent {
  ...
  userFetchdatabyId(event: any) {
    ...
    axios
      .get(`http://localhost:5000/userfetchbyId/${selecteduserId}`)
      .then((response) => {
  ...
  userrequest(event: any) {
    this.user_id = event;
    axios
      .patch(`http://localhost:5000/userrequestaccept/${this.user_id}`)
      .then((response) => {
  ...
  userrejectedbyadmin(event: any) {
    this.user_id = event;
    axios
      .patch(`http://localhost:5000/userrequestreject/${this.user_id}`)
      .then((response) => {
        alert(response.data.message);
        this.userverificationdetails();
        this.userverificationForm.reset();
